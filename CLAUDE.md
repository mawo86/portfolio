...
- `outputs/xlsx/ki-readiness-check-bewertungsraster.xlsx` (neu, 2026-09-21) — digitales Bewertungsraster: 3 Blätter (Anleitung/Prozesse/Bewertung), rechnet Std./Woche-Umrechnung, Score und Phase (Sofort/3 Monate/Beobachten) automatisch aus, plus automatisch generiertes Blasendiagramm (Automatisierbarkeit × Umsetzbarkeit, Größe = Score, erste 5 Zeilen). Bindeglied zwischen den Papier-Bögen (Rohdaten-Erfassung) und dem PDF-Report (Ergebnis). Formeln und Chart gegen Testdaten aus dem Verdichtungs-Training verifiziert (LibreOffice headless, Rendering-Kontrolle). Achsenwahl der Excel-Grafik weicht bewusst von der PDF-S.4-Matrix ab (Methodik Abschnitt 5) — nur zum Arbeiten, nicht 1:1 fürs Kunden-Deliverable.
