...
- Tonflächen als Bedeutungsträger (`src/lib/tonal.ts`, seit 2026-09-26): sechs Pastelltöne `tonal.blue/green/yellow/red/purple/teal` plus `-900`-Textfarben in `tailwind.config.mjs`, dazu `neutral`. Feste Zuordnung: Pakete (`paketTonal`: Check blau, Pilot grün, Begleitung gelb, Tagessatz neutral), Unternehmensbereiche (`bereichTonal`, z. B. Vertrieb blau, Kundenservice gelb, Backoffice grün, HR lila, Einkauf petrol, Produktion rot) und Blog-Kategorien (`kategorieTonal`). Helfer `chipBereich()`, `chipKategorie()`, `chipFor()` liefern fertige Chip-Klassen; der Finder bekommt die Klasse über `toFinderItem().bereichChip` ins JSON. Neue Bereiche oder Kategorien brauchen einen Eintrag in der jeweiligen Map, sonst fällt der Chip auf neutral zurück.
- `PaketKachel.astro` ersetzt auf `/leistungen` die Paketbilder: Tonfläche mit Stufennummer als Wasserzeichen und je einer paketspezifischen Mini-Grafik (7 Leitfragen und drei Phasen beim Check, vier Wochenbalken beim Pilot, Monatsraster mit zwei Terminen bei der Begleitung, Stundenleiste beim Tagessatz). Die Startseite zeigt die drei Stufen als Tonkarten statt als Liste.
- Kleine Überraschungen: rotierendes Lösungsbeispiel unter den Hero-Buttons (`#hero-beispiel`, aus den vier Top-Lösungen, respektiert `prefers-reduced-motion`), Kontakt-Sektion als blaue Fläche mit weißer Formularkarte, Lesefortschritts-Linie in `BlogLayout`, animierter Haken auf `/danke`, die 404-Seite zeigt eine "Lösungskarte ohne Lösung", Pill-Buttons drücken sich beim Klick leicht ein (globale Regel in `Layout.astro`).
...
**Higgsfield-Visuals:** Die Seite sucht zur Build-Zeit nach Dateien in `website/public/media/` (`src/lib/media.ts` → `hasMedia()`). Fehlt eine Datei, rendert `MediaImage.astro` einen ruhigen Verlauf. Einziger verbliebener Slot: `about.jpg` (Über mich). Seit 2026-09-26 liegt dort keine KI-Fotografie mehr, sondern eine selbst gerenderte Grafik in den Designfarben (Workflow Anfrage → Prüfung → Angebot, 1120×1400, erzeugt per Chromium-Screenshot aus einer HTML-Vorlage, Quelle nicht im Repo). Ein echtes Porträt kann sie jederzeit ersetzen, dann `alt` in `index.astro` anpassen. Hero-Video, Paketbilder (`paket-*.jpg`) und Kontakt-Hintergrund (`kontakt.jpg`) sind mit dem Redesign entfallen, die Prompts in `reference/higgsfield-briefing.md` dazu sind hinfällig.
...
**Blog:** 36 Artikel live, keine Drafts (alle 22 Pipeline-Artikel am 2026-09-26 reviewt und veröffentlicht, danach zwei neue Artikel zur SAP-nahen Nische). Blog-Index-Seite (`/blog`) hat NYT-inspiriertes Editorial-Design mit Kategorie-Navigation und Pagination. Artikel-Layout (`BlogLayout.astro`) mit Lesezeit, JSON-LD, Prev/Next-Navigation, RelatedPosts, Newsletter-CTA und Autor-Karte. `@tailwindcss/typography` ist installiert und aktiviert.
...
- Dritte Welle (2026-09-26, nach `outputs/content-strategie.md` und Nischen-Fokus aus `context/strategy.md`): `angebot-aus-anfrage-mit-ki` (How-to, Kategorie Automatisierung, führt zur Lösung `angebotsentwurf-aus-anfrage`) und `rechnungseingang-technik-vergleich` (Tool-Vergleich XML-Parser / Dokumenten-KI / Sprachmodell / Suite, Kategorie Tools & Setup, führt zur Lösung `rechnungseingang-vorerfassung`). Beide verlinken Lösungsbibliothek und mindestens zwei Artikel, Zahlen stammen aus `outputs/loesungswege/_belege.md`.
...
**Prompt: Lösungen belegen:** `outputs/prompt-loesungen-belegen.md` (2026-09-26), **ausgeführt am selben Tag**, Bericht in `outputs/loesungen-belegen-bericht.md`. Ergebnis:
- Alle 26 Lösungstexte folgen jetzt dem Muster Mechanismus ("Was wir bauen" beschreibt den technischen Weg), Beleg ("Was das bringt" nennt Quelle oder kennzeichnet die Zahl als Erfahrungswert) und Grenze (neuer Abschnitt "Wo es schwierig wird" vor "Was ihr dafür braucht"). Beim Schreiben neuer Lösungen diese Struktur beibehalten.
...
**Higgsfield-Assets (2026-09-19, bereinigt 2026-09-26):** Von den damals erzeugten Bildern ist keines mehr im Einsatz: `about.jpg` wurde am 2026-09-26 durch eine gerenderte Workflow-Grafik in kühlen Tönen ersetzt, weil das Konto 0 Credits hat und das warme Umgebungsbild nicht mehr zum Design passte. `paket-check.jpg`, `paket-pilot.jpg`, `paket-begleitung.jpg` wurden am 2026-09-26 gelöscht, weil `/leistungen` Tonkacheln zeigt. Erzeugt wurden sie via `image_auto` im Free-Plan (kein `gpt_image_2_5`/`recraft_v4_1`, kein 4:5-Format, Video mit rund 56 Credits nicht finanzierbar); Guthaben danach 0 Credits.
...
- `about.jpg` durch ein echtes Porträt ersetzen (aktuell gerenderte Workflow-Grafik als Übergangslösung); Higgsfield-Konto hat 0 Credits, ein KI-Foto braucht Top-up
...
- Neue Blog-Artikel schreiben: Die Pipeline ist leer, Rhythmus laut `outputs/content-strategie.md` 2x/Monat. Nächste Kandidaten aus der Lösungsbibliothek: Ticket-Klassifikation, SAP-SD-Auftragsprüfung, Wissensdatenbank mit Quellenbeleg
